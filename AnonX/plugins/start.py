...
async def start_comm(client, message: Message, _):
    ...
    if len(message.text.split()) > 1:
        ...
        if name[0:4] == "help":
            ...
            await message.reply_sticker("CAACAgUAAxkBAAEBdxhjtyB8AXMlYwXhq28KQVDYP0OzmAACTgYAAgGIqVUJOrEb7kjYoC0E")
        ...
        if name == "verify":
            await message.reply_text(f"ʜᴇʏ {message.from_user.first_name},\nᴛʜᴀɴᴋs ʙᴀʙʏ 💋❤ ғᴏʀ ᴠᴇʀɪғʏɪɴɢ ʏᴏᴜʀsᴇʟғ ɪɴ {config.MUSIC_BOT_NAME}, ɴᴏᴡ ʏᴏᴜ ᴄᴀɴ ɢᴏ ʙᴀᴄᴋ ᴀɴᴅ sᴛᴀʀᴛ ᴜsɪɴɢ ᴍᴇ ᴡɪᴛʜᴏᴜᴛ ᴀɴʏ ᴘʀᴏʙʟᴇᴍ.")
            if await is_on_off(config.LOG):
                sender_id = message.from_user.id
                sender_name = message.from_user.first_name
                return await app.send_message(
            ...
            key = InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(
                            text="• sᴜᴩᴩᴏʀᴛ •", url="https://t.me/User_Ankit"
                        ),
                    ],
                ]
            )
